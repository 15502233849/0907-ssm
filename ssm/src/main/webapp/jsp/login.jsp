	  ...
	  $("#loginForm").validate({
		   rules:{
			   username:{
				   required:true,
				   pattern:/^\w{5,8}$/,
				   remote:"UserServlet?m=validUsername"
			   },
			   password:{
				   required:true,
				   pattern:/^\w{5,8}$/,
				   remote:{
					   url:"UserServlet?m=validPassword",
				       type:"post",
				       dataType:"json",
				       data:{
				    	   username:function(){
		   ...
		   messages:{
			   username:{
				   pattern:"用户名必须是 5到8位的字母数字或者下划线组成",
				   remote:"该用户名不存在"
			   },
			   password:{
				   pattern:"密码必须是 5到8位的字母数字或者下划线组成",
				   remote:"密码错误"
			   }
		   }
	   });
		...
		<div class="span12 last">
			<div class="wrap">
				<div class="main">
					...
					<form id="loginForm" action="${ pageContext.request.contextPath }/UserServlet?m=login"  method="post" novalidate="novalidate">
						<table>
							<tbody><tr>
								<th>
										用户名:
							...
							<tr class="register">
								...
								<td>
									<dl>
										...
										<dd>
											立即注册即可体验在线购物！
											<a href="${ pageContext.request.contextPath }/jsp/regist.jsp">立即注册</a>
										</dd>
									</dl>
								</td>
							</tr>
