...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">
			function del(id){
				if(confirm("你确认删除吗?")){
				    window.location.href = "${pageContext.request.contextPath }/AdminServlet?m=del&id="+id;
				}
			}
			function add(){
			...
			function query(){
				var name=$("#name").val();
				window.location.href = "${pageContext.request.contextPath}/AdminServlet?m=list&name="+name;
			}
			function fenye(curPage){
				var name=$("#name").val();
				window.location.href = "${pageContext.request.contextPath}/AdminServlet?m=list&name="+name+"&curPage="+curPage;
			}
			function assignRole(id){
				$("#tb tr:gt(0)").remove();
				$.post(
				   "${pageContext.request.contextPath}/adminRole/selRoleByAid.do",
				   {id:id},
				   function(obj){
					  var row="";
			...
			function save(id){
				...
				if(ids.length>1){
					ids=ids.substr(1);
					$.post(
						"${pageContext.request.contextPath}/adminRole/saveRole.do",
						{id:id,ids:ids},
						function(obj){
							alert(obj?"ok":"err");
							$("#tb").hide();
...
&#28155;&#21152;
					...
					<tr>
						<td class="ta_01" align="center" bgColor="#f5fafe">
						    <div>
						    请输入要查询的名字:<input type="text" name="name" id="name" value="${username}"/>
									...
									<c:forEach items="${list}" var="entry" varStatus="status">
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td align="center" style="HEIGHT: 22px">
												<a href="${ pageContext.request.contextPath }/AdminServlet?m=getById&id=${entry.id}">
