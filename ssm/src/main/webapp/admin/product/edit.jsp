...
<HTML>
	...
	<body>
		<!--  -->
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/AdminProductServlet?m=update" method="post" enctype="multipart/form-data">
			...
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						<input type="text" name="name" id="name" value="${product.name}"  class="bg"/>
					</td>
					<td width="18%" align="center" bgColor="#f5fafe" class="ta_01">
						是否热门：
					</td>
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						...
						<c:if test="${!empty product.image}"><img src="${pageContext.request.contextPath}/${product.image}"/></c:if>
					</td>
				</tr>
				<tr>
					<td width="18%" align="center" bgColor="#f5fafe" class="ta_01">
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						<select name="scid">
							<c:forEach items="${seconds}" var="second">
								<option value="${second.id}" <c:if test="${second.id==product.scid}">selected</c:if>>${second.name}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
