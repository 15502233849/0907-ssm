...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">
			function del(id){
				if(confirm("你确认删除吗?")){
					location.href ="${pageContext.request.contextPath}/AdminCategoryServlet?m=del&id="+id;
				}
			}
			...
			function query(){
				var name=$("#name").val();
				window.location.href = "${pageContext.request.contextPath}/AdminCategoryServlet?m=list&name="+name;
			}
			function fenye(curPage){
				var name=$("#name").val();
				window.location.href = "${pageContext.request.contextPath}/AdminCategoryServlet?m=list&name="+name+"&curPage="+curPage;
			}
		</script>
	</HEAD>
	<body>
									...
									<c:forEach items="${list}" var="entry" varStatus="status">
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td align="center" style="HEIGHT: 22px">
												<a href="${ pageContext.request.contextPath }/AdminCategoryServlet?m=getById&id=${entry.id}">
